...
<form action="" method="get">
	<div class="input-group col-md-5 col-md-offset-7">
		...
		//	while($p=mysql_fetch_array($pil)){
			...
			$stmt = $user_home->runQuery("SELECT DISTINCT tanggal FROM barang_laku ORDER BY tanggal desc");
			$stmt->execute();
			if($stmt->rowCount()>0)
		{
...
<table class="table">
	...
	$stmt = $user_home->runQuery("SELECT * FROM barang_laku WHERE tanggal like :tanggal ORDER BY tanggal desc");
	$stmt->execute(array(":tanggal"=>$tanggal));
 
		
		
	}else{
		//$brg=mysql_query("select * from barang_laku order by tanggal desc");
		$stmt = $user_home->runQuery("SELECT * FROM barang_laku ORDER BY tanggal desc");
		$stmt->execute();
	...
	<tr>
		...
		$tanggal = trim($_GET['tanggal']);	
		$stmt = $user_home->runQuery("SELECT SUM(total_harga) as total FROM barang_laku WHERE tanggal =:tanggal");
		$stmt->execute(array(":tanggal"=>$tanggal));
		$rp = $stmt->fetch(PDO::FETCH_ASSOC);
			...
			echo "<td><b> Rp.". number_format($rp['total']).",-</b></td>";
		}else{
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			...
			//
		$tanggal = trim($_GET['tanggal']);	
		$stmt = $user_home->runQuery("SELECT SUM(laba) as total FROM barang_laku WHERE tanggal =:tanggal");
		$stmt->execute(array(":tanggal"=>$tanggal));
		$rp = $stmt->fetch(PDO::FETCH_ASSOC);
			//
			echo "<td><b> Rp.". number_format($rp['total']).",-</b></td>";
