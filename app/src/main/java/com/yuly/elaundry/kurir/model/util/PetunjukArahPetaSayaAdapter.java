...
public class PetunjukArahPetaSayaAdapter extends RecyclerView.Adapter<PetunjukArahPetaSayaAdapter.ViewHolder> {
...
//            this.time = (TextView) itemView.findViewById(R.id.nav_instruction_item_time_tv);
        ...
        public void setItemData(Instruction itemData) {
                        icon.setImageResource(PetaSayaNavigasi.getNavigator().getPetunjukArah(itemData));
            description.setText(PetaSayaNavigasi.getNavigator().getDirectionDescription(itemData));
            distance.setText(PetaSayaNavigasi.getNavigator().getDistance(itemData));
//            time.setText(String.valueOf(Navigator.getNavigator().getTime(itemData)));
        }
    }

    // Provide a suitable constructor (depends on the kind of dataset)
    public PetunjukArahPetaSayaAdapter(InstructionList instructions) {
        this.instructions = instructions;
    }

    // Create new views (invoked by the layout manager)
    @Override public PetunjukArahPetaSayaAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        // create a new view
